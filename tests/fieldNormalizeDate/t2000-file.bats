...
@test "normalizing first field to default date format" {
    LC_ALL=C run -0 fieldNormalizeDate -F $'\t' 1 -- "${BATS_TEST_DIRNAME}/tabbed.txt"
    assert_output - <<'EOF'
Wed Apr 20 00:00:00 CEST 2022	foo	@1649663333
...
@test "normalizing first field to epoch with --format" {
    LC_ALL=C run -0 fieldNormalizeDate -F $'\t' 1 --format -%s- -- "${BATS_TEST_DIRNAME}/tabbed.txt"
    assert_output - <<'EOF'
-1650405600-	foo	@1649663333
-1649663364-	now	@1111111111
-1640991600-	new year	@1111111111
...
@test "normalizing first field and last field to default date format" {
    LC_ALL=C run -0 fieldNormalizeDate -F $'\t' 1 -1 -- "${BATS_TEST_DIRNAME}/tabbed.txt"
    assert_output - <<'EOF'
Wed Apr 20 00:00:00 CEST 2022	foo	Mon Apr 11 09:48:53 CEST 2022
...
@test "normalizing first field and last field to custom date formats that create an additional field" {
    LC_ALL=C run -0 fieldNormalizeDate -F $'\t' 1 '[%U/%Y]' -1 $'on %F\t%R' -- "${BATS_TEST_DIRNAME}/tabbed.txt"
    assert_output - <<'EOF'
[16/2022]	foo	on 2022-04-11	09:48
