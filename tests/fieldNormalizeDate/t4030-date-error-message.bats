...
@test "invalid date in first field gets replaced with message with --date-error message" {
    LC_ALL=C run -0 --separate-stderr fieldNormalizeDate -F $'\t' --date-error message 1 -- "${BATS_TEST_DIRNAME}/bad.txt"
    assert_output - <<'EOF'
date: invalid date '01-01-1000'	foo	@1649663333
...
@test "replacing invalid date with --date-error message also applies to following field" {
    LC_ALL=C run -0 --separate-stderr fieldNormalizeDate -F $'\t' --date-error message 1 -1 -- "${BATS_TEST_DIRNAME}/bad.txt"
    assert_output - <<'EOF'
date: invalid date '01-01-1000'	foo	Mon Apr 11 09:48:53 CEST 2022
...
@test "replacing invalid date with --date-error message can be reverted to abort for following field" {
    LC_ALL=C run -1 --separate-stderr fieldNormalizeDate -F $'\t' --date-error message 1 --date-error abort -1 -- "${BATS_TEST_DIRNAME}/bad.txt"
    assert_output "date: invalid date '01-01-1000'	foo	Mon Apr 11 09:48:53 CEST 2022"
}

@test "replacing invalid date with --date-error message can be changed to ignore for following field" {
    LC_ALL=C run -0 --separate-stderr fieldNormalizeDate -F $'\t' --date-error message 1 --date-error ignore -1 -- "${BATS_TEST_DIRNAME}/bad.txt"
    assert_output - <<'EOF'
date: invalid date '01-01-1000'	foo	Mon Apr 11 09:48:53 CEST 2022
Mon Apr 11 09:49:24 CEST 2022	now	""
date: invalid date 'not a date'	new year	neither this
