...
@test "turning first field to 0 and filtering lines without 0 or 0 via coproc" {
    run -0 fieldMap -F $'\t' --filter "|$filterNonO0Command" 1 0 "${BATS_TEST_DIRNAME}/tabbed.txt"
    assert_output - <<'EOF'
0	first	100	A Here
0	no4	201
0
...
@test "filtering lines without 0 or o via coproc" {
    run -0 fieldMap -F $'\t' --filter "|$filterNonO0Command" "${BATS_TEST_DIRNAME}/tabbed.txt"
    assert_output - <<'EOF'
...
@test "filtering lines where the first field has no 0 or o via coproc" {
    run -0 fieldMap -F $'\t' --filter "\$1|$filterNonO0Command" "${BATS_TEST_DIRNAME}/tabbed.txt"
    assert_output - <<'EOF'
foo	first	100	A Here
eof
EOF
