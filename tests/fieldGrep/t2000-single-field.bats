...
@test "grep the second field with a regexp yields two lines" {
    run -0 fieldGrep --input "${BATS_TEST_DIRNAME}/tabbed.txt" -F $'\t' --regexp '^.....$' 2
    assert_output - <<'EOF'
foo	first	100	A Here
baz	third	333	C U
EOF
...
@test "grep the last field with a non-matching pattern yields nothing and exits with 1" {
    run -1 fieldGrep --input "${BATS_TEST_DIRNAME}/tabbed.txt" -F $'\t' --regexp doesNotMatch -1
    assert_output ''
}
